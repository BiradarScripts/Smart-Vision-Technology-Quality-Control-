...
const parseAnalysis = (analysis: string): Omit<ExpiryItem, 'id'>[] => {
  const items = analysis.split(/Item Number:/g).filter(Boolean)
  return items.map(item => {
    const lines = item.split('\n').filter(line => line.trim() !== "")
    const result: Partial<ExpiryItem> = {}
    ...
    return result as Omit<ExpiryItem, 'id'>
  })
}
...
export default function UploadExpiryDetectionPage() {
  ...
  const handleContinue = async () => {
    if (image) {
      ...
      try {
        ...
        const analysisResults = parsedAnalyses.map(item => ({
          ...item,
          id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
        }))

        localStorage.setItem('expiryDetectionResults', JSON.stringify(analysisResults))
        ...
        {isCameraOpen ? (
          ...
          <div className="space-y-6 w-full max-w-[500px]">
            ...
            >
              ...
              >
                Learn More About Expiry Detection
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
            </Link>
          </div>
