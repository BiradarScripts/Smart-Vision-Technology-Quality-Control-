...
export default function UploadFreshnessDetectionPage() {
  ...
  const handleContinue = async () => {
    if (image) {
      ...
      try {
        ...
        const analysisResults = parsedAnalyses.map(analysis => ({
          ...analysis,
  ...
  const parseAnalysis = (analysis: string): Omit<AnalysisResult, 'id'>[] => {
    ...
    return items.map(item => {
      ...
      lines.forEach(line => {
        ...
        switch (key.trim().toLowerCase()) {
          ...
          case 'freshness index':
            result.freshnessIndex = parseInt(value)
            break
          case 'status':
            result.status = value
            break
          case 'visual color':
            result.visualColor = value
            break
          case 'surface texture':
            result.texture = value
            break
          case 'firmness level':
            result.firmness = value
            break
          case 'packaging condition':
            result.packagingCondition = value
            break
          case 'estimated shelf life':
            result.estimatedShelfLife = value
            break
          case 'recommendation':
      ...
      return result as Omit<AnalysisResult, 'id'>
    })
  }
        ...
        {isCameraOpen ? (
          ...
          <div className="space-y-6 w-full max-w-[500px]">
            ...
            >
              ...
              >
                Learn More About Freshness Detection
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
            </Link>
          </div>
